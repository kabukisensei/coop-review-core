    ...
    *,
    ...
    for r in agent_review:
        items.append({
            ...
            "message": str(r.get("note", "agent review required"))
        })
        
    # include only error diagnostics per SARIF convention
    for d in diagnostics:
        if d.severity == "error":
            items.append({
                "type": "diagnostic",
    ...
    # determine failures: error severity
    failures = sum(1 for it in items if it["severity"] == "error")
    skipped = sum(1 for it in items if it["severity"] == "warning")
    
    lines = [
        '<?xml version="1.0" encoding="UTF-8"?>',
        f'<testsuites name={quoteattr(tool_name)} tests="{len(items)}" failures="{failures}" skipped="{skipped}" errors="0" time="0">'
    ]
    
    suite_name = f"{tool_name} v{version}"
    lines.append(f'  <testsuite name={quoteattr(suite_name)} tests="{len(items)}" failures="{failures}" skipped="{skipped}" errors="0" time="0">')
    
    for it in sorted(items, key=lambda x: (x["file"], x["line"], x["rule_id"], x["message"])):
        cls_name = it["rule_id"]
        test_name = f'{it["file"]}:{it["line"]}'
        msg = it["message"]
        lines.append(f'    <testcase classname={quoteattr(cls_name)} name={quoteattr(test_name)} time="0">')
        if it["severity"] == "error":
            lines.append(f'      <failure message={quoteattr(msg)} type="error">{escape(msg)}</failure>')
        elif it["severity"] == "warning":
            lines.append(f'      <skipped message={quoteattr("warning: " + msg)}/>')
        else:
            lines.append(f'      <!-- info: {escape(msg)} -->')
