...
| `report.py` | the shared report layer (issue #9): console chrome (`BADGE`/`BADGE_COLOR`/`ANSI`/`sty`), the branded `HTML_STYLE` + `logo_data_uri` (the ONE bundled `data/cooptimize-logo.png`), `esc`/`chip`, the machine-JSON envelope (`verdict`, `build_envelope`, `envelope_text`, `diagnostic_json`, `log_text`), the SARIF 2.1.0 emitter (`to_sarif`, `sarif_location`, `SARIF_LEVEL`, `SARIF_FINGERPRINT_KEY`), and the JUnit XML emitter (`to_junit` — for Azure DevOps' native `PublishTestResults@2`). Renders from plain data — never a tool's `Result`. |
